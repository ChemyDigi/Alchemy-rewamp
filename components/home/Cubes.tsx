...
export interface CubesProps {
  ...
  rippleColor?: string;
  rippleSpeed?: number;
  images?: string[];           // flat array — one image per cube, in row-major order
  faceImageSize?: 'cover' | 'contain' | 'fill';
}
...
const depth = cubeSize ? cubeSize / 2 : 0;
  const rowGap =
    typeof cellGap === 'number'
      ? `${cellGap}px`
  ...
  // Convert flat array index → image URL, cycling if fewer images than cubes
const getImage = (r: number, c: number): string | undefined => {
  if (!images || images.length === 0) return undefined;
  const idx = r * gridSize + c;
  return images[idx]; // ❌ NO modulo → no repeating
};
          ...
          cells.map((__, c) => {
            ...
            return (
              ...
              >
                ...
                {/* FRONT FACE — unique image per cube */}
              {/* FRONT */}
<div
